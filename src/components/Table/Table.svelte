<script lang='ts'>
    ...
    function calculaItemInicialPagina(pagina: number, itensPorPagina: number): number {
        const isPaginaInicial: boolean = pagina === 1;
        return isPaginaInicial ? 1 : (pagina * itensPorPagina + 1 - 10);
    }

    function calculaItemFinalPagina(pagina: number, itensPorPagina: number, total: number): number {
        const isPaginaInicial: boolean = pagina === 1;
    ...
    function getItensPorPagina(pagina: number, itensPorPagina: number, total: number) {
        ...
        return `${calculaItemInicialPagina(pagina, itensPorPagina)}-${calculaItemFinalPagina(pagina, itensPorPagina, total)}`;
    }
</script>
...
<div class='overflow-x-auto shadow-md sm:rounded-lg w-full'>
    ...
    {#if promiseResolved}
        <nav class='flex items-center justify-between p-4' aria-label='Table navigation'>
            ...
            >Apresentando <span
                class='font-semibold text-gray-900 dark:text-white'>{getItensPorPagina(fetchApiData.page, fetchApiData.per_page, fetchApiData.count)}</span> de
            ...
            <ul class='inline-flex items-center -space-x-px'>
                <li>
                    <a
                        ...
                        class="{Number(fetchApiData.page) === 1
                ...
                {#each Array(Math.ceil(fetchApiData.count / fetchApiData.per_page)) as page, index}
                    <li>
                        <a
                            ...
                            class={Number(fetchApiData.page) === index + 1
                ...
                <li>
                    <a
                        href='#'
                        on:click={() => handleChangePage(Number(fetchApiData.page) + 1, 10)}
                        class="{Number(fetchApiData.page) === Math.ceil(fetchApiData.count / fetchApiData.per_page)
                            ? 'bg-gray-200 pointer-events-none'
